/**
 * A fragment shader for processing an image to blur it vertically.
 */
...
void main() {
  mediump vec2 pixelHeight = vec2(0.0, 1.0) / uSamplerSize;
  ...
  result += texture2D(uSampler, vTextureCoord + pixelHeight * 1.0).rgb * WEIGHT_1;
  result += texture2D(uSampler, vTextureCoord - pixelHeight * 1.0).rgb * WEIGHT_1;
  result += texture2D(uSampler, vTextureCoord + pixelHeight * 2.0).rgb * WEIGHT_2;
  result += texture2D(uSampler, vTextureCoord - pixelHeight * 2.0).rgb * WEIGHT_2;
  result += texture2D(uSampler, vTextureCoord + pixelHeight * 3.0).rgb * WEIGHT_3;
  result += texture2D(uSampler, vTextureCoord - pixelHeight * 3.0).rgb * WEIGHT_3;
  result += texture2D(uSampler, vTextureCoord + pixelHeight * 4.0).rgb * WEIGHT_4;
  result += texture2D(uSampler, vTextureCoord - pixelHeight * 4.0).rgb * WEIGHT_4;
